...
import uuid
import json
import label_vocabulary_generation.labels.visionlabeler
import numpy
import math
import label_vocabulary_generation.labels.threadedvision
import time
...
def run_json(json_path):
    ...
    for items in loaded_json["items"]:
        ...
        id, annotations = label_vocabulary_generation.labels.visionlabeler.label_image_online(full_path, info[0])
        #print('labels image')
        label_vocabulary_generation.labels.visionlabeler.record_labels(id, annotations, db)
        db.conn.commit()
...
def batch_thread_db(paths):
    ...
    lbls = label_vocabulary_generation.labels.threadedvision.download_all_sites(infos)
